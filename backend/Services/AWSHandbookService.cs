...
{
    public class AWSHandbookService : IAWSService
    {
        private readonly IAmazonS3 _s3Client;
        private readonly string S3Bucket;
        private readonly string S3BucketHandbookFolder;

        public AWSHandbookService(IConfiguration config, IAmazonS3 s3Client)
        {
            _s3Client = s3Client;
            S3Bucket = config.GetValue<string>("AWS:S3Bucket");
            S3BucketHandbookFolder = config.GetValue<string>("AWS:S3BucketHandbookFolder");
        }
        ...
        public async Task<bool> DeleteFileFromS3Async(string path)
        {
            try
            {
                string keyName = $"{S3BucketHandbookFolder}/{path}";
